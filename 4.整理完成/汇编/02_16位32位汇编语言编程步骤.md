...
②下载上述链接中的压缩包，解压缩后，生成masm32文件夹，里面包含了32位汇编语言的编译程序(xp虚拟机中32位汇编编译器已预装在d:\masm32, 不需要做第②步);
...
(5) masm h02; 把h02.asm编译成h02.obj,末尾有分号
(6) link h02; 把h02.obj连接成h02.exe,末尾有分号
(7) h02        运行h02.exe
(8) exit       关闭dos窗口
...
如何调试汇编语言程序?
(1)dos环境下16位汇编程序的调试
①xp虚拟机的命令行(开始->运行->command)对h02.exe进行调试:
masm h02;
link h02;
td h02.exe
其中td的全名为Turbo Debugger。TD中按F8单步跟踪。
在td数据窗中按ctrl+g，再输入ds:dx即可观察ds:dx指向的内容。

②dosbox86命令行(file->dos shell) 对h02.exe进行调试:
masm h02;
link h02;
td h02.exe
③dosbox86集成环境中对h02.exe进行调试:
在dosbox86集成环境中选菜单debug可以对16位汇编语言程序进行调试:
在完成编译连接后点菜单debug->debug,再点菜单view->cpu。
...
该格式中mov指令的方向是从左到右。
linux下的$pc相当于windows下的eip
eip: extended instruction pointer
eip或pc总是指向将要执行的指令。
linux下编译、运行、调试hello的过程如下:
