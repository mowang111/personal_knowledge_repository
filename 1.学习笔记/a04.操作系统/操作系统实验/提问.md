...
## 问题：
背景：在Linux 0.11上通过创建TCB而实现内核级线程，实现`pthread_create(pthread_t* thread, const pthread_attr_t* attr, void* start_routine, void* arg)`系统调用。
下面是我的实现方式：
函数接口部分：
```c
...
int sys_pthread_create_kernel(pthread_t* thread, const pthread_attr_t* attr, void* start_routine){
    ...
    q->tss.cs = main_thread->tss.cs & 0xffff;
    ...
    put_fs_long(q->tid,thread);
    cli();
    return 0;
}
```

其中，`thread_struct`是仿照`task_struct`进行的改造：
```c
struct task_struct {
	long signal;
...
在测试过程中，当新建完成一个线程，再次调用`scanf("%d", &cmd);`后，会产生GPF报错，提示的报错点在schedule函数中，我很不理解为什么会产生这个报错，如何进一步调试